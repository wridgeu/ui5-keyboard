...
**Fix applied**: For non-docked `Full` keyboards, the KioskKeyboard maintains consistent height across all switchable layouts by tracking the maximum rendered height and applying it as `min-height`. This prevents the content resize that triggers the Popover's buggy reposition logic. As a side benefit, this matches the behavior of real-world virtual keyboards (iOS, Android, ATMs) where the keyboard area is a fixed zone and keys rearrange within it. Docked keyboards are excluded from this behavior because they pin to the viewport edge, where minimising footprint is more valuable than preventing layout shifts.
