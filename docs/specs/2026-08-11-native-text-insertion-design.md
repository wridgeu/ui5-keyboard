...
- **A live Hangul preedit can exceed the target's `maxlength`; the commit clamps it.** The same direct assignment writes the preedit past a `maxlength` the field advertises, so a `maxlength="2"` target visibly holds three syllables for as long as composition continues. Enforcement sits at the commit instead: `commitComposition` re-inserts the committed text through `insertText`, whose clamp reduces an overflowing syllable to nothing, so the value a consumer reads is always within the limit (`kiosk-keyboard-webc/test/component/native-insertion.test.ts` pins it). Clamping the preedit itself is not available at this layer: `updateComposition` would have to truncate text whose jamo the `hangul-compose` state machine still holds, leaving `phase`/`curL`/`curV`/`curT` describing characters that are not in the DOM. Refusing the jamo that would overflow is the option that keeps the two in agreement, at the cost of a keypress that shows nothing. `kana-dakuten` is unaffected — it composes through `insertText` over a range and is therefore already clamped.
