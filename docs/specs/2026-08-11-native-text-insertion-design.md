...
- **A key activated from the physical keyboard takes the fallback, and that discards the stack the pointer built.** `execCommand` needs the target focused, and a keycap reached through the roving tabindex holds focus itself: kiosk `onsapselect` (`KioskKeyboard.ts:2171`) and webc `key-grid-navigation.ts:121` (Enter) / `:166` (Space on release) all activate without returning focus to the target first. Measured in Chromium on the web component: type `a` by pointer with the target focused, then activate `b` from a focused keycap — the field reads `"ab"` and `execCommand("undo")` reverts nothing, because the assignment discarded the pointer edit's entry (§2.4). The same two keys both activated by pointer undo to `"a"`. The kiosk path is the same guard under the same activation shape. So the undo half of this fix is pointer-driven only, and mixing the two input modes is worse than either alone; `maxlength` holds on both paths regardless (§3.5). Focusing the target first is not the fix — it would take focus off the grid and break the navigation the keys are being reached with. Out of scope for #230.
...
- **A live Hangul preedit honours `maxlength` by refusing the key, not by truncating (#273, resolved).** The direct assignment is not policed by `maxlength`, so the limit has to be applied before the write. Truncating inside `updateComposition` is still not available at this layer — it would cut text whose jamo the `hangul-compose` state machine still holds, leaving `phase`/`curL`/`curV`/`curT` describing characters that are not in the DOM. `hangul-compose` therefore refuses the key at the five seams that open a new preedit (`hasRoomForPreedit`), which is the option that keeps the machine and the DOM in agreement.

  The refusal is deliberate divergence from the engines, on a ground that does not generalise. WebKit and Blink both ship `LayoutTests/fast/forms/input-maxlength-ime-preedit.html` ("This test checks users can input pre-edit text longer than maxlength"), and Gecko closed bug 1164361 and bug 1167095 as INVALID over it, with the rationale in bug 717600 comment 2: "The composition string must not be cut off by maxlength attribute because Japanese IME needs to be able to convert longer unconverted text to shorter converted text." A phonetic buffer is allowed to overrun because it converts _down_ to fewer characters on commit. This middleware's preedit is exactly one code unit and commits as that same unit, so it can never convert down — an overrun is not a buffer awaiting conversion, it is a keystroke guaranteed to be dropped. Measured in Chromium on `test/pages/index.html` before the fix, `maxlength="2"`, tapping ㅎㅏㄴㄱㅜㄱㅁㅏ: the field showed `한국마` and the commit silently ate the 마, so two keypresses produced nothing after appearing to work. The guard belongs in the middleware and not in `updateComposition` for the same reason: consumer middleware whose composition genuinely does shrink must keep the engines' latitude.

  Enforcement at the commit is unchanged and still load-bearing: `commitComposition` re-inserts through `insertText`, whose clamp covers the text the guard does not gate.

- **A composition replaces the target's selection (resolved alongside #273).** `startComposition` took `selectionStart` only, and `updateComposition` derives its range from that offset alone, so a preedit opened over a selection was inserted at the selection start and the selected text survived beside it — `ABCD` with `BC` selected became `A가BCD`, where a plain typed character correctly gives `AXD` through `insertText`. `startComposition` now removes the selected range before recording `preeditStart`. This is also what lets `hasRoomForPreedit` credit the selection as room rather than refusing a jamo on a full field the composition is about to clear.

- `kana-dakuten` is unaffected by both — it composes through `insertText` over a range, so it is already clamped and already replaces its range, and it holds no state machine that could describe absent text.
