...
- **H3 (width injectivity is live).** If `keyWidthClass` becomes lossy (two widths collide on one output), the guard must fail. Perturbation: make kiosk `keyWidthClass` return a constant. Expected: red on the injectivity branch.
- **H4 (a missing contract fails loudly, not empty).** If a contract module cannot be imported (renamed/moved), the guard must error out with a non-zero exit, not silently pass having compared nothing. Perturbation: import a nonexistent path. Expected: throw, non-zero exit.
...
| H3         | kiosk `keyWidthClass` returns constant | red: `keyWidthClass is not injective over KeyWidth ...` (exit 1)                | yes     |
| H4         | import a bogus contract path           | threw `ERR_MODULE_NOT_FOUND`, exit 1 (not a silent empty pass)                  | yes     |

All four cleared on 2026-07-21; each perturbation reverted immediately after observing red.
