...
- **H4 (platform-only allowlist is live).** A key one twin is allowed to carry alone must not be silently accepted on the other. If a webc-only class appears on kiosk, the guard must fail. Perturbation: add the webc-only `variantPopupHost` class to kiosk `classes`. Expected: red on the "kiosk ... unclassified" branch. (After the 2026-07-22 symmetry revert there are no platform-only _attributes_, so this hypothesis is exercised through the class allowlist.)

## Results

Re-verified 2026-07-22 after the symmetry revert (category is a class on both twins; no platform-only attributes); each perturbation reverted immediately after observing red.
