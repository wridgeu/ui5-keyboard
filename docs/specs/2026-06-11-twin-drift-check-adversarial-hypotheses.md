...
| H5  | The pair manifest silently shrinks (e.g. layouts list hardcoded and a glob/typo drops entries), so the check passes while comparing fewer pairs.  | The script asserts the expected pair count (16 layouts + 5 core modules = 21) and prints it; a manifest edit that drops one must be loud. | count printed; mismatch fails        |
