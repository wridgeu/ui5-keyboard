...
- Date: 2026-07-21
- Status: design, implemented then revised (kiosk for correctness, webc for symmetry)
- Superseded for the key category on BOTH twins. This design made the key category a `data-key-type` attribute; every claim to that effect below (section 4.1, the specificity rule in 4.4, the "attributes gain `keyType` / classes lose `keyModifier`+`keyAction` on both packages" contract change in 4.5, and the "assert `data-key-type` identical" expectation in 5 and 8) no longer holds. On kiosk the attribute was reverted for correctness (a scoped attribute selector is `(0,2,0)` and buries the state-indicator rules). On webc it was reverted for symmetry (the attribute was a `(0,1,0)` behavioral no-op with no consumer hook, so keeping it only made the contract asymmetric). The category is now the `keyModifier`/`keyAction` classes on both twins; `keyType` does not exist. `data-key-span`, `data-fkey`, and `data-glyph-script` shipped as designed (attributes on both twins; the kiosk glyph rules are scoped under `.ui5KioskKey__label`). See `2026-07-21-dom-contract-kiosk-lightdom-specificity.md` for the analysis, the family-by-family split, and the 2026-07-22 symmetry follow-up.
...
- Rebased onto main (2026-07-23), which had since landed #192: the webc height tier moved from the `hostCqShort`/`hostCqTiny` host classes to a `cq-tier` host attribute (the light-DOM kiosk twin keeps its `rootCqShort`/`rootCqTiny` classes). References below to webc `hostCqShort`/`hostCqTiny` (the PLATFORM_ONLY webc list in section 5, and the PR #188 note above) describe the pre-rebase contract; the shipped drift guard now classifies `cq-tier` as a webc-only attribute and the tier classes as kiosk-only.
