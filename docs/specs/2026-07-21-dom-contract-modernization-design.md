...
- Superseded in part for the KIOSK twin. This design made the key category a `data-key-type` attribute on both twins; every claim to that effect below (section 4.1, the specificity rule in 4.4, the "attributes gain `keyType` / classes lose `keyModifier`+`keyAction` on both packages" contract change in 4.5, and the "assert `data-key-type` identical" expectation in 5 and 8) holds for **webc** but not for the kiosk light DOM, where a scoped attribute selector is `(0,2,0)` and buries the state-indicator rules. On kiosk, `data-key-type` was reverted to the `keyModifier`/`keyAction` classes, so `keyType` is a webc-only attribute and the guard lists `keyModifier`/`keyAction` as kiosk `PLATFORM_ONLY`. `data-key-span`, `data-fkey`, and `data-glyph-script` shipped as designed (attributes on both twins; the kiosk glyph rules are scoped under `.ui5KioskKey__label`). See `2026-07-21-dom-contract-kiosk-lightdom-specificity.md` for the analysis and the family-by-family split that shipped.
