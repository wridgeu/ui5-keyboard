...
Slot JSDoc does not require `@default`: the tag is allowed on slots, but the missing-default check runs on the property branch only.
...
`patches/@ui5+webcomponents-tools+2.26.0.patch`, applied by the root `postinstall` through `patch-package`, changes the UI5 CEM config and the analyzer sources that `@ui5/webcomponents-tools` patches in turn. What this document describes is the patched behaviour; the stock tooling differs in five places:

- **Accessor-backed properties resolve a type**: the `tsProgramMember` lookup accepts `GetAccessorDeclaration` and `SetAccessorDeclaration` alongside `PropertyDeclaration`, which is what gives `open`, `disabled`, `keyboardType`, `mobileKeyboard` and `fKeyMode` an attribute type.
- **Static-aware JSDoc matching**: a member is matched to its source node by name **and** `static`, so a static and an instance member sharing a name cannot take each other's JSDoc.
- **Rest parameters**: `parameter.rest` is set for a `...` parameter, so `composeLayout(...sources)` is described as a rest parameter rather than a single positional one.
- **Parameter order**: the output sorter skips `parameters` and `mixins`, so method parameters keep declaration order instead of being sorted by name.
- **`{@link}` in descriptions**: an inline link renders as its label, or as the target when it carries no label, instead of the target and the label concatenated.
...
**Allowed getter tags**: `public`, `protected`, `private`, `since`, `deprecated`, `formEvents`, `formProperty`, `default`, `override`
