...
4. **Navigation**: Moves the caret for the arrow, `Home`/`End` and `PageUp`/`PageDown` keys. When the press carries Shift, the selection's focus end moves and its anchor is kept instead. Which end is the anchor is not stored anywhere: it is the browser's `selectionDirection` (`"backward"` puts the focus at `selectionStart`; `"forward"` and `"none"` put it at `selectionEnd`), written back on every extending press and read on the next one.
...
The source is held by `LayoutState` (`internal/layout-state.ts`) and read via `getSource()`; `clearUserOverride()` drops a user pick, source back to `"external"` and layout back to the tracked base, on a target switch and on every `keyboardType` change, so a lifted constraint lands on the base (a pick of a primary layout is itself the base and stays).
