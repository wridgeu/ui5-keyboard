...
Wrapping a row in CSS is the right tool when the wrapped arrangement is the row's own order, read left to right and top to bottom, and vertical arrow movement inside the row does not matter. The row stays one logical row, so ArrowLeft/ArrowRight walk it in order while ArrowUp/ArrowDown skip the wrap (see [Rows: Choose the Arrangement](#rows-choose-the-arrangement-dont-reflow-it)).
...
This wraps `toolRow` into Cut/Copy/Paste over Undo/Redo/Find - the row's own order, so nothing moves past anything else. ArrowDown from Cut moves on to the first QWERTY row rather than to Undo; if it should reach Undo, split the toolbar into two rows of layout data instead.
