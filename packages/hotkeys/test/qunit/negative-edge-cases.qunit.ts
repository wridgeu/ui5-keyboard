...
// One row per `_assertAlive` call site in HotkeyManager, so the guard's coverage
// is readable as a list rather than inferred from seven near-identical tests.
// `register()` appears once, not twice: `_assertAlive` is its first statement,
// ahead of any sequence parsing, so the "G I" spelling exercises nothing the
// plain one does not.
