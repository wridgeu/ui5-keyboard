...
describe("kiosk-keyboard", () => {
  ...
  describe("backspace auto-repeat", () => {
    ...
    it("does not swallow a keyboard/programmatic Backspace after sliding off and releasing off-key", async () => {
      ...
      // Slide the pointer off the key (relatedTarget outside it), then release
      // off-key: no Backspace click is generated by the gesture.
      bksp.dispatchEvent(
        new PointerEvent("pointerout", { bubbles: true, composed: true, pointerId: 1, relatedTarget: input }),
      );
      releasePointer();
