...
| --------------------- | ----------------- | ---------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `layout`              | `layout`          | `string`               | `""`        | Layout **asked for** (e.g. `qwerty`, `qwertz-de`). Empty = auto-detect from locale. Not rewritten by what renders; read `effectiveLayout` for that. A secondary layout named here (`numeric`, `special`, ...) renders, while `{layout:base}` and `reset()` return to the locale layout. |
...
| `keyboard-type`       | `keyboardType`    | `string`               | `"Full"`    | `"Full"`, `"Numpad"`, or `"Numeric"`. Any value other than `"Full"` disables auto-type detection until `resetKeyboardType()`.                                                                                                                                                           |
...
| `resetKeyboardType()`      | Resets keyboard type to `"Full"` and re-enables auto-type detection. Fires `keyboard-type-change` with `autoDetected: false`; a `keyboardType` assigned from inside that handler stands.              |
...
Docked keyboards default to `64rem` (1024px at the default root font-size) max-width and center automatically via `margin-inline: auto`. Their bottom padding adds `env(safe-area-inset-bottom)`, so the last row clears a home indicator or gesture bar on a page that opts into `viewport-fit=cover`.
