...
export function foldCustomLayouts(
  ...
  for (const spec of specs) {
    ...
    // `@property() keycapLang = ""` is typed but never coerced, and `isCustomLayout` is a
    // duck-typed check, so `el.keycapLang = 42` or a `toSpec()` from a foreign bundle reaches
    // this line unvalidated. `spec.keycapLang?.trim()` would throw and lose the entire custom
    // layout instead of dropping one facet. The kiosk twin coerces at the setter
    // (`ManagedObject.validateProperty` does `oValue = "" + oValue`), so there this is unreachable
    // through the aggregation and the guard stands for parity and for hand-built specs.
    // oxlint-disable-next-line anti-slop/no-runtime-typeof
    ...
    // `@property() compact = ""` is typed but never coerced, and `isCustomLayout` is a
    // duck-typed check, so `el.compact = 42` or a `toSpec()` from a foreign bundle reaches this
    // line unvalidated. `spec.compact?.trim()` would throw and lose the entire custom layout
    // instead of dropping one facet. The kiosk twin coerces at the setter
    // (`ManagedObject.validateProperty` does `oValue = "" + oValue`), so there this is unreachable
    // through the aggregation and the guard stands for parity and for hand-built specs.
    // oxlint-disable-next-line anti-slop/no-runtime-typeof
    ...
    const patch: LayoutMeta = {
      ...
      // the tier below standing. `CustomLayout.toSpec()` emits `secondary` only for a declared
      // `layoutRole`, so the reachable non-boolean is a duck-typed foreign `toSpec()`, which
      // `isCustomLayout` admits.
      // oxlint-disable-next-line anti-slop/no-runtime-typeof
    ...
    if (spec.middleware !== undefined) {
      // `middleware` is an uncoerced `@property({ type: Object })`, and the host accepts any
      // duck-typed `toSpec()` through `isCustomLayout`, so a non-callable does reach this line
      // and would throw at composition time. The fold's contract is to report it as
      // `invalid-middleware` and keep going. The kiosk twin has UI5's `type: "function"`
      // validation ahead of it, so there the same check is belt and braces.
      // oxlint-disable-next-line anti-slop/no-runtime-typeof
