...
> **Why only `@container` and `@layer`?** The vendored LESS 1.6.3 has other gaps compared to modern CSS (e.g., `&` is not resolved inside `:not()`, making nested selectors like `&--cq-short:not(&--numpad)` output invalid CSS). We chose not to patch the parent-selector resolution because it would require changes throughout the parser's selector compilation pipeline -- significantly more invasive than adding two case labels to a switch statement. `@container` and `@layer` follow the existing directive pattern exactly, making them safe and minimal patches. For `&`-in-`:not()`, we work around it by writing the full class name instead. The full fix belongs in an upstream LESS version update.
